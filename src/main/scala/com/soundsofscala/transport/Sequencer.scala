  ...
  private def playMusicalEvent(
    ...
    musicalEvent match
      case Melody(head, tail) =>
        playMusicalEvent(head, tempo, instrument) >> IO.sleep(
          head.durationToFiniteDuration(tempo)) >> playMusicalEvent(tail, tempo, instrument)
...
object Sequencer:
  private def playSynth(note: Note, tempo: Tempo, instrument: ScalaSynth): IO[Unit] =
    IO(println(s"Playing note: $note")) *>
      IO(instrument.attackRelease(note, tempo))
