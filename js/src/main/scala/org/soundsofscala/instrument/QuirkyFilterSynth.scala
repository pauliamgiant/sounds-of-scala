...
object QuirkyFilterSynth:
  def apply()(using audioContext: AudioContext): IO[QuirkyFilterSynth] =
    for
      activeNodesRef <- Ref.of[IO, Set[AudioNode]](Set.empty)
    yield new QuirkyFilterSynth(activeNodesRef)

final class QuirkyFilterSynth private (
  ...
  override def attackRelease(
    ...
    for
      createdNodes <- IO {
        ...
        println(
          s"QuirkyFilterSynth: Playing note ${note.frequency} at velocity $velocity, volume $volume, final: $velocityModulatedVolume")
